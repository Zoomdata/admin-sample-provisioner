...
.action(function(args, cb) {
     ...
     async.each(array, function(user, callback) {
       createAccount('accounts', user.username, 'supervisor', 'Z00mda1a')
       callback(
         async.nextTick(function() {
           addUserToAccountName('accounts/', user.username, user.username + 'user1', 'supervisor', 'Z00mda1a');
         })
       );
     })
     cb();
...
vorpal
  ...
  .action(function(args, cb) {
    ...
    var username = args.username;
    var password = args.password;
    addUserToAccountName('accounts/', accountName, newAccountName, 'supervisor', 'Z00mda1a');
    cb();
  })
